...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nammpp in configuration namelist', lwp )
   ...
   SUBROUTINE mppsend( ktyp, pmess, kbytes, kdest, md_req )
      ...
      SELECT CASE ( cn_mpi_send )
      CASE ( 'S' )                ! Standard mpi send (blocking)
         CALL mpi_send ( pmess, kbytes, mpi_real, kdest , ktyp, mpi_comm_opa        , iflag )
      CASE ( 'B' )                ! Buffer mpi send (blocking)
         CALL mpi_bsend( pmess, kbytes, mpi_real, kdest , ktyp, mpi_comm_opa        , iflag )
      CASE ( 'I' )                ! Immediate mpi send (non-blocking send)
         ! be carefull, one more argument here : the mpi request identifier..
         CALL mpi_isend( pmess, kbytes, mpi_real, kdest , ktyp, mpi_comm_opa, md_req, iflag )
      END SELECT
      !
   ...
   SUBROUTINE mpprecv( ktyp, pmess, kbytes, ksource )
      ...
      !
      CALL mpi_recv( pmess, kbytes, mpi_real, use_source, ktyp, mpi_comm_opa, istatus, iflag )
      !
   ...
   SUBROUTINE mppgather( ptab, kp, pio )
      ...
      !
      itaille = jpi * jpj
      CALL mpi_gather( ptab, itaille, mpi_real, pio, itaille     ,   &
         &                            mpi_real, kp , mpi_comm_opa, ierror )
      !
   ...
   SUBROUTINE mppscatter( pio, kp, ptab )
      ...
      itaille = jpi * jpj
      !
      CALL mpi_scatter( pio, itaille, mpi_real, ptab, itaille     ,   &
         &                            mpi_real, kp  , mpi_comm_opa, ierror )
      !
   ...
   SUBROUTINE mppmax_a_real( ptab, kdim, kcom )
      ...
      IF( PRESENT(kcom) ) localcomm = kcom
      !
      CALL mpi_allreduce( ptab, zwork, kdim, mpi_real, mpi_max, localcomm, ierror )
      ptab(:) = zwork(:)
      !
   ...
   SUBROUTINE mppmax_real( ptab, kcom )
      ...
      IF( PRESENT(kcom) )   localcomm = kcom
      !
      CALL mpi_allreduce( ptab, zwork, 1, mpi_real, mpi_max, localcomm, ierror )
      ptab = zwork
      !
   ...
   SUBROUTINE mppmax_real_multiple( ptab, NUM , kcom  )
      ...
      IF( PRESENT(kcom) )   localcomm = kcom
      !
      CALL mpi_allreduce( ptab, zwork, NUM, mpi_real, mpi_max, localcomm, ierror )
      ptab = zwork
   ...
   SUBROUTINE mppmin_a_real( ptab, kdim, kcom )
      ...
      IF( PRESENT(kcom) ) localcomm = kcom
      !
      CALL mpi_allreduce( ptab, zwork, kdim, mpi_real, mpi_min, localcomm, ierror )
      ptab(:) = zwork(:)
      !
   ...
   SUBROUTINE mppmin_real( ptab, kcom )
      ...
      IF( PRESENT(kcom) )   localcomm = kcom
      !
      CALL mpi_allreduce( ptab, zwork, 1, mpi_real, mpi_min, localcomm, ierror )
      ptab = zwork
      !
   ...
   SUBROUTINE mppsum_a_real( ptab, kdim, kcom )
      ...
      IF( PRESENT(kcom) )   localcomm = kcom
      !
      CALL mpi_allreduce( ptab, zwork, kdim, mpi_real, mpi_sum, localcomm, ierror )
      ptab(:) = zwork(:)
      !
   ...
   SUBROUTINE mppsum_real( ptab, kcom )
      ...
      IF( PRESENT(kcom) ) localcomm = kcom
      !
      CALL mpi_allreduce( ptab, zwork, 1, mpi_real, mpi_sum, localcomm, ierror )
      ptab = zwork
      !
   ...
   SUBROUTINE mppsum_realdd( ytab, kcom )
      ...
      CALL MPI_ALLREDUCE (ytab, zwork, 1, MPI_COMPLEX, MPI_SUMDD, localcomm, ierror )
      ytab = zwork
      !
   ...
   SUBROUTINE mppsum_a_realdd( ytab, kdim, kcom )
      ...
      IF( PRESENT(kcom) )   localcomm = kcom
      !
      CALL MPI_ALLREDUCE( ytab, zwork, kdim, MPI_COMPLEX, MPI_SUMDD, localcomm, ierror )
      ytab(:) = zwork(:)
      !
   ...
   SUBROUTINE mpp_minloc2d( ptab, pmask, pmin, ki,kj )
      ...
      zain(2,:)=ki+10000.*kj
      !
      CALL MPI_ALLREDUCE( zain,zaout, 1, MPI_2REAL,MPI_MINLOC,MPI_COMM_OPA,ierror)
      !
      pmin = zaout(1,1)
   ...
   SUBROUTINE mpp_maxloc2d( ptab, pmask, pmax, ki, kj )
      ...
      zain(2,:) = ki + 10000. * kj
      !
      CALL MPI_ALLREDUCE( zain,zaout, 1, MPI_2REAL,MPI_MAXLOC,MPI_COMM_OPA,ierror)
      !
      pmax = zaout(1,1)
      kj   = INT( zaout(2,1) / 10000.     )
      ki   = INT( zaout(2,1) - 10000.* kj )
   ...
   SUBROUTINE mpp_lbc_north_3d( pt3d, cd_type, psgn )
      ...
      ELSE
         CALL MPI_ALLGATHER( znorthloc  , itaille, MPI_REAL,                &
            &                znorthgloio, itaille, MPI_REAL, ncomm_north, ierr )
         !
         ztab(:,:,:) = 0.e0
   ...
   SUBROUTINE mpp_lbc_north_2d_multiple( pt2d_array, cd_type, psgn, num_fields)
      ...
      ELSE
         !
         CALL MPI_ALLGATHER( znorthloc  , itaille*num_fields, MPI_REAL,        &
            &                znorthgloio, itaille*num_fields, MPI_REAL, ncomm_north, ierr )
         !
         ztab(:,:,:) = 0.e0
         DO jk = 1, num_fields
            DO jr = 1, ndim_rank_north            ! recover the global north array
   ...
   SUBROUTINE mpp_lbc_north_e( pt2d, cd_type, psgn)
      ...
      CALL MPI_ALLGATHER( znorthloc_e(1,1)  , itaille, MPI_REAL,    &
         &                znorthgloio_e(1,1,1), itaille, MPI_REAL, ncomm_north, ierr )
      !
      DO jr = 1, ndim_rank_north            ! recover the global north array
         iproc = nrank_north(jr) + 1
         ildi = nldit (iproc)
