...
!   INTEGER, PUBLIC, PARAMETER ::   MPI_DOUBLE_PRECISION = 8
   INTEGER, PUBLIC, PARAMETER ::   MPI_DOUBLE_PRECISION = 4 ! not used
...
# endif
   ...
   SUBROUTINE mppgather( ptab, kp, pio )
      ...
      CALL mpi_gather( ptab, itaille, mpi_real, pio, itaille     ,   &
         &                            mpi_real, kp , mpi_comm_oce, ierror )
#else
      pio(:,:,1) = ptab(:,:)
#endif
      !
   ...
   SUBROUTINE mpp_delay_sum( cdname, cdelay, y_in, pout, ldlast, kcom )
     !!----------------------------------------------------------------------
      ...
      IF( ndelayid(idvar) == -1 ) THEN         ! first call without restart: define %y1d and %z1d from y_in with blocking allreduce
         ...
         CALL mpi_allreduce( y_in(:), todelay(idvar)%y1d(:), isz, MPI_COMPLEX, mpi_sumdd, ilocalcomm, ierr )   ! get %y1d
...
# if defined key_mpi2
      ...
      CALL  mpi_allreduce( y_in(:), todelay(idvar)%y1d(:), isz, MPI_COMPLEX, mpi_sumdd, ilocalcomm, ndelayid(idvar), ierr )
      IF( ln_timing ) CALL tic_tac(.FALSE., ld_global = .TRUE.)
# else
      CALL mpi_iallreduce( y_in(:), todelay(idvar)%y1d(:), isz, MPI_COMPLEX, mpi_sumdd, ilocalcomm, ndelayid(idvar), ierr )
# endif
#else
      pout(:) = REAL(y_in(:), wp)
#endif
   ...
   SUBROUTINE mpp_delay_max( cdname, cdelay, p_in, pout, ldlast, kcom )
      ...
      IF( ndelayid(idvar) == -1 ) THEN         ! first call without restart: define %z1d from p_in with a blocking allreduce
         ...
         CALL mpi_allreduce( p_in(:), todelay(idvar)%z1d(:), isz, MPI_REAL, mpi_max, ilocalcomm, ierr )   ! get %z1d
      ENDIF
...
# if defined key_mpi2
      IF( ln_timing ) CALL tic_tac( .TRUE., ld_global = .TRUE.)
      CALL  mpi_allreduce( p_in(:), todelay(idvar)%z1d(:), isz, MPI_REAL, mpi_max, ilocalcomm, ndelayid(idvar), ierr )
      IF( ln_timing ) CALL tic_tac(.FALSE., ld_global = .TRUE.)
# else
      CALL mpi_iallreduce( p_in(:), todelay(idvar)%z1d(:), isz, MPI_REAL, mpi_max, ilocalcomm, ndelayid(idvar), ierr )
# endif
#else
      pout(:) = p_in(:)
#endif
