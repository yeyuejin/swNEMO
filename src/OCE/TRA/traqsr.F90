...
CONTAINS
   ...
   SUBROUTINE tra_qsr( kt )
      ...
      !!----------------------------------------------------------------------
# if defined qsr_slave
      integer, external :: slave_qsr4
      type :: qsr_var
         integer   :: jpi, jpj, jpk, fs_two, fs_jpim1, jpjm1, jpkm1, narea, nksr, jp_tem
         real*8    :: zz0, xsi0r, zz1, xsi1r, z1_2
         integer*8 :: loc_gdepw_n, loc_qsr, loc_wmask, loc_qsr_hc_b, loc_e3t_n, loc_qsr_hc, loc_tsa
      end type qsr_var
      type(qsr_var) :: q_v
      q_v%jpi = jpi
      q_v%jpj = jpj
      q_v%jpk = jpk
      q_v%fs_two = fs_2
      q_v%fs_jpim1 = fs_jpim1
      q_v%jpjm1 = jpjm1
      q_v%jpkm1 = jpkm1
      q_v%narea = narea
      q_v%nksr = nksr
      q_v%jp_tem = jp_tem
      q_v%loc_gdepw_n = loc(gdepw_n)
      q_v%loc_qsr = loc(qsr)
      q_v%loc_wmask = loc(wmask)
      q_v%loc_qsr_hc_b = loc(qsr_hc_b)
      q_v%loc_e3t_n = loc(e3t_n)
      q_v%loc_qsr_hc = loc(qsr_hc)
      q_v%loc_tsa = loc(tsa)
# endif
      !
...
# if defined qsr_slave
      ...
      q_v%xsi1r = xsi1r
      q_v%z1_2 = z1_2
      call athread_spawn(slave_qsr4, q_v)
      call athread_join()
!
...
!               tmp1(ji,jj,jk) = qsr(ji,jj) * ( zc0 * wmask(ji,jj,jk) - zc1 * wmask(ji,jj,jk+1) ) 
!               tmp2(ji,jj,jk) = tmp2(ji,jj,jk)   &
!                  &                 + z1_2 * ( qsr_hc_b(ji,jj,jk) + tmp1(ji,jj,jk) ) / e3t_n(ji,jj,jk)
!            END DO
!         END DO
!      END DO
!
