...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: eosbn2.F90 11993 2019-11-28 10:20:53Z cetlod $
...
# if defined slave
      ...
      var_z%r1_T0 = r1_T0
      var_z%r1_Z0 = r1_Z0
      var_z%rdeltaS = rdeltaS
      var_z%r1_rau0 = r1_rau0
      var_z%loc_pts = loc(pts)
      var_z%loc_pdep = loc(pdep)
      var_z%loc_tmask = loc(tmask)
      var_z%loc_prd = loc(prd)
      var_z%EOS013 = EOS013; var_z%EOS103 = EOS103; var_z%EOS003 = EOS003
      var_z%EOS022 = EOS022; var_z%EOS112 = EOS112; var_z%EOS012 = EOS012
      var_z%EOS202 = EOS202; var_z%EOS102 = EOS102; var_z%EOS002 = EOS002
      var_z%EOS041 = EOS041; var_z%EOS131 = EOS131; var_z%EOS031 = EOS031
      var_z%EOS221 = EOS221; var_z%EOS121 = EOS121; var_z%EOS021 = EOS021
      var_z%EOS311 = EOS311; var_z%EOS211 = EOS211; var_z%EOS111 = EOS111
      var_z%EOS011 = EOS011; var_z%EOS401 = EOS401; var_z%EOS301 = EOS301
      var_z%EOS201 = EOS201; var_z%EOS101 = EOS101; var_z%EOS001 = EOS001
      var_z%EOS060 = EOS060; var_z%EOS150 = EOS150; var_z%EOS050 = EOS050
      var_z%EOS240 = EOS240; var_z%EOS140 = EOS140; var_z%EOS040 = EOS040
      var_z%EOS330 = EOS330; var_z%EOS230 = EOS230; var_z%EOS130 = EOS130
      var_z%EOS030 = EOS030; var_z%EOS420 = EOS420; var_z%EOS320 = EOS320
      var_z%EOS220 = EOS220; var_z%EOS120 = EOS120; var_z%EOS020 = EOS020
      var_z%EOS510 = EOS510; var_z%EOS410 = EOS410; var_z%EOS310 = EOS310
      var_z%EOS210 = EOS210; var_z%EOS110 = EOS110; var_z%EOS010 = EOS010
      var_z%EOS600 = EOS600; var_z%EOS500 = EOS500; var_z%EOS400 = EOS400
      var_z%EOS300 = EOS300; var_z%EOS200 = EOS200; var_z%EOS100 = EOS100
      var_z%EOS000 = EOS000
# endif             
...
# endif
# if defined slave
         call athread_spawn(slave_insitu, var_z)
         call athread_join()
# else
      SELECT CASE( neos )
      !
         ...
         DO jk = 1, jpkm1
            DO jj = 1, jpj
               DO ji = 1, jpi
                  ...
                  ytmp1 =  ABS( pts(ji,jj,jk,jp_sal) + rdeltaS )
                  ytmp2 = ytmp1 * r1_S0
                  zs = SQRT(ytmp2)
...
# if defined insitu_time
         if(lwp) then
           call cyc_time(ed)
           elapsed = ed - st
           qbw = 8*2.25*(jpi*jpj*jpk*5)/((elapsed-60000)*45.0)
         end if
# endif
# if defined insitu_time
         if(lwp) then
           print *, 'insitu_time: ', elapsed, ' BW: ', qbw
         endif
# endif
...
# if defined slave
      ...
      pot_v%r1_Z0 = r1_Z0
      pot_v%r1_T0 = r1_T0
      pot_v%rdeltaS = rdeltaS
      pot_v%r1_S0 = r1_S0
      pot_v%r1_rau0 = r1_rau0
      pot_v%loc_pdep = loc(pdep)
      pot_v%loc_pts = loc(pts)
      pot_v%loc_tmask = loc(tmask)
      pot_v%loc_prhop = loc(prhop)
      pot_v%loc_prd = loc(prd)
      pot_v%EOS000 = EOS000; pot_v%EOS131 = EOS131
      pot_v%EOS001 = EOS001; pot_v%EOS140 = EOS140
      pot_v%EOS002 = EOS002; pot_v%EOS140 = EOS140
      pot_v%EOS003 = EOS003; pot_v%EOS150 = EOS150
      pot_v%EOS010 = EOS010; pot_v%EOS200 = EOS200
      pot_v%EOS011 = EOS011; pot_v%EOS201 = EOS201
      pot_v%EOS012 = EOS012; pot_v%EOS202 = EOS202
      pot_v%EOS013 = EOS013; pot_v%EOS210 = EOS210
      pot_v%EOS020 = EOS020; pot_v%EOS211 = EOS211
      pot_v%EOS021 = EOS021; pot_v%EOS220 = EOS220
      pot_v%EOS022 = EOS022; pot_v%EOS221 = EOS221
      pot_v%EOS030 = EOS030; pot_v%EOS230 = EOS230
      pot_v%EOS031 = EOS031; pot_v%EOS240 = EOS240
      pot_v%EOS040 = EOS040; pot_v%EOS300 = EOS300
      pot_v%EOS041 = EOS041; pot_v%EOS301 = EOS301
      pot_v%EOS050 = EOS050; pot_v%EOS310 = EOS310
      pot_v%EOS060 = EOS060; pot_v%EOS311 = EOS311
      pot_v%EOS100 = EOS100; pot_v%EOS320 = EOS320
      pot_v%EOS101 = EOS101; pot_v%EOS330 = EOS330
      pot_v%EOS102 = EOS102; pot_v%EOS400 = EOS400
      pot_v%EOS103 = EOS103; pot_v%EOS401 = EOS401
      pot_v%EOS110 = EOS110; pot_v%EOS410 = EOS410
      pot_v%EOS111 = EOS111; pot_v%EOS420 = EOS420
      pot_v%EOS112 = EOS112; pot_v%EOS500 = EOS500
      pot_v%EOS120 = EOS120; pot_v%EOS510 = EOS510
      pot_v%EOS121 = EOS121; pot_v%EOS600 = EOS600
      pot_v%EOS130 = EOS130
# endif
      !
...
# endif
      ...
      !   ELSE
# if defined slave
         call athread_spawn(slave_pot1, pot_v)
         call athread_join()
# else
            DO jk = 1, jpkm1
               DO jj = 1, jpj
...
# if defined rab_time
      ...
      common /flag/ print_flag
# endif
      real(wp) :: ytmp1, ytmp2, ytmp3, ytmp4

# if defined slave
      integer, external :: slave_rab1
      type :: rab_var
         integer :: jpi, jpj, jpk, jpkm1, narea, jp_tem, jp_sal
         real(wp) :: r1_Z0, r1_T0, rdeltaS, r1_S0, r1_rau0
         integer*8 :: loc_gdept_n, loc_pts, loc_tmask, loc_pab
         real(wp) :: ALP000, ALP001, ALP002, ALP003, ALP010, ALP011, ALP012, ALP020, ALP021, ALP030, ALP031, ALP040, ALP050, ALP100, ALP101, ALP102, ALP110, ALP111, ALP120, ALP121, ALP130, ALP140, ALP200, ALP201, ALP210, ALP211, ALP220, ALP230, ALP301, ALP310, ALP320, ALP400, ALP410, ALP500, ALP300
         real(wp) :: BET000, BET001, BET002, BET003, BET010, BET011, BET012, BET020, BET021, BET030, BET031, BET040, BET050, BET100, BET101, BET102, BET110, BET111, BET120, BET121, BET130, BET140, BET200, BET201, BET210, BET211, BET220, BET230, BET301, BET310, BET320, BET400, BET410, BET500, BET300
      end type rab_var
      type(rab_var) :: r_v
      r_v%jpi = jpi
      r_v%jpj = jpj
      ...
      r_v%r1_S0 = r1_S0
      r_v%r1_rau0 = r1_rau0
      r_v%loc_gdept_n = loc(gdept_n)
      r_v%loc_pts = loc(pts)
      r_v%loc_tmask = loc(tmask)
      r_v%loc_pab = loc(pab)
      
      r_v%ALP000 = ALP000;  r_v%BET000 = BET000
      r_v%ALP001 = ALP001;  r_v%BET001 = BET001
      r_v%ALP002 = ALP002;  r_v%BET002 = BET002
      r_v%ALP003 = ALP003;  r_v%BET003 = BET003
      r_v%ALP010 = ALP010;  r_v%BET010 = BET010
      r_v%ALP011 = ALP011;  r_v%BET011 = BET011
      r_v%ALP012 = ALP012;  r_v%BET012 = BET012
      r_v%ALP020 = ALP020;  r_v%BET020 = BET020
      r_v%ALP021 = ALP021;  r_v%BET021 = BET021
      r_v%ALP030 = ALP030;  r_v%BET030 = BET030
      r_v%ALP031 = ALP031;  r_v%BET031 = BET031
      r_v%ALP040 = ALP040;  r_v%BET040 = BET040
      r_v%ALP050 = ALP050;  r_v%BET050 = BET050
      r_v%ALP100 = ALP100;  r_v%BET100 = BET100
      r_v%ALP101 = ALP101;  r_v%BET101 = BET101
      r_v%ALP102 = ALP102;  r_v%BET102 = BET102
      r_v%ALP110 = ALP110;  r_v%BET110 = BET110
      r_v%ALP111 = ALP111;  r_v%BET111 = BET111
      r_v%ALP120 = ALP120;  r_v%BET120 = BET120
      r_v%ALP121 = ALP121;  r_v%BET121 = BET121
      r_v%ALP130 = ALP130;  r_v%BET130 = BET130
      r_v%ALP140 = ALP140;  r_v%BET140 = BET140
      r_v%ALP200 = ALP200;  r_v%BET200 = BET200
      r_v%ALP201 = ALP201;  r_v%BET201 = BET201
      r_v%ALP210 = ALP210;  r_v%BET210 = BET210
      r_v%ALP211 = ALP211;  r_v%BET211 = BET211
      r_v%ALP220 = ALP220;  r_v%BET220 = BET220
      r_v%ALP230 = ALP230;  r_v%BET230 = BET230
      r_v%ALP301 = ALP301;  r_v%BET301 = BET301
      r_v%ALP310 = ALP310;  r_v%BET310 = BET310
      r_v%ALP320 = ALP320;  r_v%BET320 = BET320
      r_v%ALP400 = ALP400;  r_v%BET400 = BET400
      r_v%ALP410 = ALP410;  r_v%BET410 = BET410
      r_v%ALP500 = ALP500;  r_v%BET500 = BET500
      r_v%ALP300 = ALP300;  r_v%BET300 = BET300
# endif
      !
...
# endif
      ...
      !   !
# if defined slave
         call athread_spawn(slave_rab1, r_v)
         call athread_join()
# else
         DO jk = 1, jpkm1
            DO jj = 1, jpj
...
!                  zs  = SQRT( ABS( pts(ji,jj,jk,jp_sal) + rdeltaS ) * r1_S0 )   ! square root salinity
  ...
  !          DO jj = 1, jpj
  !             DO ji = 1, jpi
  !               if(checktmp(ji,jj,jk) .ne. pab(ji,jj,jk,jp_sal)) then
  !                 if(ytag == 0) then
  !                   print*, "slave pab err", ji,jj,jk,checktmp(ji,jj,jk), &
  !                     pab(ji,jj,jk,jp_sal), checktmp(ji,jj,jk) - pab(ji,jj,jk,jp_sal)
  !                   ytag = 1
  !                 endif
  !               endif
  !        enddo;enddo;enddo       
...
# if defined slave
      integer, external :: slave_bn21
      type :: bn2_var
         integer :: jpi, jpj, jpk, jpkm1, narea, jp_tem, jp_sal
         real(wp) :: grav
         integer*8 :: loc_gdepw_n, loc_gdept_n, loc_pab, loc_pts, loc_e3w_n, loc_wmask, loc_pn2
      end type bn2_var
      type(bn2_var) :: b_v
      b_v%jpi = jpi
      b_v%jpj = jpj
      ...
      b_v%jp_sal = jp_sal
      b_v%grav = grav
      b_v%loc_gdepw_n = loc(gdepw_n)
      b_v%loc_gdept_n = loc(gdept_n)
      b_v%loc_pts = loc(pts)
      b_v%loc_pab = loc(pab)
      b_v%loc_pn2 = loc(pn2)
      b_v%loc_e3w_n = loc(e3w_n)
      b_v%loc_wmask = loc(wmask)
# endif
      !
...
# endif
# if defined slave
      call athread_spawn(slave_bn21, b_v)
      call athread_join()
# else
         ...
         DO jj = 1, jpj          ! surface and bottom value set to zero one for all in istate.F90
            DO ji = 1, jpi
               zrw =   ( gdepw_n(ji,jj,jk  ) - gdept_n(ji,jj,jk) )   &
                  ...
                  !
               tmp1 = pab(ji,jj,jk,jp_tem) * (1. - zrw)
               tmp2 = pab(ji,jj,jk-1,jp_tem) * zrw 
               zaw = tmp1 + tmp2

               tmp1 = pab(ji,jj,jk,jp_sal) * (1. - zrw)
               tmp2 = pab(ji,jj,jk-1,jp_sal) * zrw
               zbw = tmp1 + tmp2
               !

               tmp1 = zaw * ( pts(ji,jj,jk-1,jp_tem) - pts(ji,jj,jk,jp_tem) )
               tmp2 = zbw * ( pts(ji,jj,jk-1,jp_sal) - pts(ji,jj,jk,jp_sal) )

               tmp2 = tmp1 - tmp2
               tmp1 = grav * tmp2
               tmp1 = tmp1/e3w_n(ji,jj,jk)
               pn2(ji,jj,jk) = tmp1 * wmask(ji,jj,jk)
