...
CONTAINS
  ...
  END SUBROUTINE zdf_tke
   ...
   SUBROUTINE tke_tke( pdepw, p_e3t, p_e3w, p_sh2, p_avm, p_avt )
      ...
      !!--------------------------------------------------------------------
      real(wp), dimension(jpi,jpj,jpk) :: zzz
# if defined tke_time
      real(8) :: st, ed, ss
      real(8), dimension(4) :: elapsed
      integer :: print_flag
      common /flag/ print_flag
# endif
# if defined slave
      integer, external :: slave_tke1, slave_tke2, slave_tke3, slave_tke4, slave_tke5, slave_tke6
      type :: tke_var
         integer :: jpi, jpj, jpk, fs_two, fs_jpim1, jpjm1, jpkm1, narea
         real(wp) :: rn_bshear, ri_cri, zfact1, zfact2, zfact3, rdt, rn_emin
         integer*8 :: loc_rn2b, loc_p_avm, loc_p_sh2, loc_apdlr, loc_tmask, loc_p_e3t, loc_p_e3w, loc_zd_up, loc_zd_lw, loc_zdiag, loc_dissl, loc_wmask, loc_en, loc_p_avt, loc_rn2, loc_zpelc, loc_pdepw
      end type tke_var
      type(tke_var) :: t_v
      t_v%jpi = jpi
      t_v%jpj = jpj
      t_v%jpk = jpk
      t_v%fs_two = fs_2
      t_v%fs_jpim1 = fs_jpim1
      t_v%jpjm1 = jpjm1
      t_v%jpkm1 = jpkm1
      t_v%narea = narea
      t_v%rn_bshear = rn_bshear
      t_v%ri_cri = ri_cri
      t_v%rdt = rdt
      t_v%rn_emin = rn_emin
      t_v%loc_rn2b = loc(rn2b)
      t_v%loc_p_avm = loc(p_avm)
      t_v%loc_p_sh2 = loc(p_sh2)
      t_v%loc_apdlr = loc(apdlr)
      t_v%loc_tmask = loc(tmask) 
      t_v%loc_p_e3t = loc(p_e3t)
      t_v%loc_p_e3w = loc(p_e3w)
      t_v%loc_zd_up = loc(zd_up)
      t_v%loc_zd_lw = loc(zd_lw)
      t_v%loc_zdiag = loc(zdiag)
      t_v%loc_dissl = loc(dissl)
      t_v%loc_wmask = loc(wmask)
      t_v%loc_p_avt = loc(p_avt)
      t_v%loc_en = loc(en)
      t_v%loc_rn2 = loc(rn2)
      t_v%loc_zpelc = loc(zpelc) 
      t_v%loc_pdepw = loc(pdepw)
# endif
      !
# if defined tke_time
         if(lwp) ss = MPI_WTIME()
...
# endif
# if defined slave
         call athread_spawn(slave_tke6, t_v)
         call athread_join()
# else
...
# if defined slave
      t_v%fs_jpim1 = jpim1
      call athread_spawn(slave_tke1, t_v)
      call athread_join()
# else
         DO jk = 2, jpkm1
            DO jj = 2, jpjm1
...
# if defined slave
      ...
      t_v%zfact2 = zfact2
      t_v%zfact3 = zfact3
      call athread_spawn(slave_tke2, t_v)
      call athread_join()
# else
...
# endif
# if defined slave
      call athread_spawn(slave_tke3, t_v)
      call athread_join()
# else
...
# endif      
# if defined slave
      call athread_spawn(slave_tke4, t_v)
      call athread_join()
# else
...
# endif      
# if defined slave
      call athread_spawn(slave_tke5, t_v)
      call athread_join()
# else
...
# if defined tke_time
         if(lwp .and. print_flag>0) then
             ...
             print *, 'tke_time:', elapsed, MPI_WTIME() - ss
         endif
# endif
      !
