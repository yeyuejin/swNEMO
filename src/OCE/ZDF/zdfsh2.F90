...
CONTAINS
   ...
   SUBROUTINE zdf_sh2( pub, pvb, pun, pvn, p_avm, p_sh2  ) 
...
#ifdef slave
      external :: slave_zdfsh2
      type zdf_sh2_t
          integer   :: jpi, jpj, jpk, jpjm1, jpkm1, jpim1
          integer*8 :: loc_pub, loc_pvb, loc_pun, loc_pvn, loc_e3uw_n, loc_e3uw_b, loc_p_avm, loc_e3vw_n, loc_e3vw_b, loc_wumask, loc_wvmask, loc_p_sh2, loc_umask, loc_vmask
      end type zdf_sh2_t
      type(zdf_sh2_t) :: v_sh2
      ...
      v_sh2%jpkm1       = jpkm1
      v_sh2%jpim1       = jpim1
      v_sh2%loc_pub     = loc(pub)
      v_sh2%loc_pvb     = loc(pvb)
      v_sh2%loc_pun     = loc(pun)
      v_sh2%loc_pvn     = loc(pvn)
      v_sh2%loc_e3uw_n  = loc(e3uw_n)
      v_sh2%loc_e3uw_b  = loc(e3uw_b)
      v_sh2%loc_p_avm   = loc(p_avm)
      v_sh2%loc_e3vw_n  = loc(e3vw_n)
      v_sh2%loc_e3vw_b  = loc(e3vw_b)
      v_sh2%loc_wumask  = loc(wumask)
      v_sh2%loc_wvmask  = loc(wvmask)
      v_sh2%loc_umask   = loc(umask)
      v_sh2%loc_vmask   = loc(vmask)
      v_sh2%loc_p_sh2   = loc(p_sh2)  ! 

      call athread_spawn(slave_zdfsh2, v_sh2)
      call athread_join()
#else
      !!--------------------------------------------------------------------
      !
