...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: ldfslp.F90 12198 2019-12-12 08:16:14Z smasson $
...
CONTAINS
   ...
   SUBROUTINE ldf_slp( kt, prd, pn2 )
      ...
      REAL(wp), DIMENSION(jpi,jpj,jpk) ::  zgrv, zww!, zzz
      !!----------------------------------------------------------------------
      real(wp), dimension(jpi,jpj)     :: e3un, e3vn
      real(wp), dimension(jpi,jpj)     :: gdepw_n_tmp
      real(wp) :: uslp_tmp, vslp_tmp, wslpi_tmp, wslpj_tmp 
# if defined slp_time
      real,dimension(6)::elapsed
      real :: st,ed
      integer :: print_flag
      common /flag/ print_flag
# endif
# if defined slave
      integer, external :: slave_slp1, slave_slp2, slave_slp3, slave_slp4, slave_slp5
      type :: var5
         integer :: jpi, jpj, jpk, fs_two, fs_jpim1, jpjm1, jpkm1, narea, kt
         real(wp) :: z1_slpmax, zeps, zm1_2g, z1_16, zm1_g
         integer*8 loc_zgru, loc_zgrv, loc_r1_e1u, loc_r1_e2v, loc_zdzr, loc_e3u_n, loc_e3v_n, loc_omlmask, loc_gdept_n, loc_risfdep, loc_zwz, loc_zslpml_hmlpu, loc_umask, loc_vmask, loc_zslpml_hmlpv, loc_zww, loc_e3un, loc_e3vn, loc_tmask
         integer*8 loc_pn2, loc_prd, loc_e1t, loc_e2t, loc_wmask, loc_e3w_n, loc_gdepw_n, loc_gdepw_n_tmp, loc_hmlp, loc_wslpiml, loc_wslpjml, loc_uslp, loc_vslp, loc_wslpi, loc_wslpj
      end type var5
      type(var5) :: v5
      v5%jpi = jpi
      v5%jpj = jpj
      v5%jpk = jpk
      v5%fs_two = fs_2
      v5%fs_jpim1 = fs_jpim1
      v5%jpjm1 = jpjm1
      v5%jpkm1 = jpkm1
      v5%narea = narea
      v5%kt = kt
      v5%loc_zgru = loc(zgru)
      v5%loc_zgrv = loc(zgrv)
      v5%loc_r1_e1u = loc(r1_e1u)
      v5%loc_r1_e2v = loc(r1_e2v)
      v5%loc_zdzr = loc(zdzr)
      v5%loc_e3u_n = loc(e3u_n)
      v5%loc_e3v_n = loc(e3v_n)
      v5%loc_omlmask = loc(omlmask)
      v5%loc_gdept_n = loc(gdept_n)
      v5%loc_risfdep = loc(risfdep)
      v5%loc_zwz = loc(zwz)
      v5%loc_zslpml_hmlpu = loc(zslpml_hmlpu)
      v5%loc_umask = loc(umask)
      v5%loc_vmask = loc(vmask)
      v5%loc_zslpml_hmlpv = loc(zslpml_hmlpv)
      v5%loc_zww = loc(zww)
      v5%loc_e3un = loc(e3un)
      v5%loc_e3vn = loc(e3vn)
      v5%loc_pn2 = loc(pn2)
      v5%loc_prd = loc(prd)
      v5%loc_e1t = loc(e1t)
      v5%loc_e2t = loc(e2t)
      v5%loc_wmask = loc(wmask)
      v5%loc_tmask = loc(tmask)
      v5%loc_e3w_n = loc(e3w_n)
      v5%loc_gdepw_n = loc(gdepw_n)
      v5%loc_gdepw_n_tmp = loc(gdepw_n_tmp)
      v5%loc_hmlp = loc(hmlp)
      v5%loc_wslpiml = loc(wslpiml)
      v5%loc_wslpjml = loc(wslpjml)
      v5%loc_uslp = loc(uslp)
      v5%loc_vslp = loc(vslp)
      v5%loc_wslpi = loc(wslpi)
      v5%loc_wslpj = loc(wslpj)
# endif
      !
...
# endif
# if defined slave
      call athread_spawn(slave_slp1, v5)
      call athread_join()
# else
...
# endif
# if defined slave
      call athread_spawn(slave_slp2, v5)
      call athread_join()
# else
...
# if defined slave
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1   ! vector opt.
            e3un(ji,jj) = e3u_n(ji,jj,miku(ji,jj))
            e3vn(ji,jj) = e3v_n(ji,jj,mikv(ji,jj))
         END DO
      END DO
      call athread_spawn(slave_slp3, v5)
      call athread_join()
# else
...
# if defined slave
      DO jj = 2, jpjm1
         DO ji = fs_2, fs_jpim1
            gdepw_n_tmp(ji,jj) = gdepw_n(ji,jj,mikt(ji,jj))
         END DO
      END DO
      call athread_spawn(slave_slp4, v5)
      call athread_join()
# else
      DO jk = 2, jpkm1
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               uslp_tmp = z1_16 * (        zwz(ji-1,jj-1,jk) + zwz(ji+1,jj-1,jk)      &
                  &                 +      zwz(ji-1,jj+1,jk) + zwz(ji+1,jj+1,jk)      &
                  &                 + 2.*( zwz(ji  ,jj-1,jk) + zwz(ji-1,jj  ,jk)      &
                  &                 +      zwz(ji+1,jj  ,jk) + zwz(ji  ,jj+1,jk) )    &
                  &                 + 4.*  zwz(ji  ,jj  ,jk)                       )
               vslp_tmp = z1_16 * (        zww(ji-1,jj-1,jk) + zww(ji+1,jj-1,jk)      &
                  &                 +      zww(ji-1,jj+1,jk) + zww(ji+1,jj+1,jk)      &
                  &                 + 2.*( zww(ji  ,jj-1,jk) + zww(ji-1,jj  ,jk)      &
                  &                 +      zww(ji+1,jj  ,jk) + zww(ji  ,jj+1,jk) )    &
                  &                 + 4.*  zww(ji,jj    ,jk)                       )

               uslp(ji,jj,jk) = uslp_tmp * ( umask(ji,jj+1,jk) + umask(ji,jj-1,jk  ) ) * 0.5_wp   &
                  &                      * ( umask(ji,jj  ,jk) + umask(ji,jj  ,jk+1) ) * 0.5_wp
               vslp(ji,jj,jk) = vslp_tmp * ( vmask(ji+1,jj,jk) + vmask(ji-1,jj,jk  ) ) * 0.5_wp   &
                  &                      * ( vmask(ji  ,jj,jk) + vmask(ji  ,jj,jk+1) ) * 0.5_wp
            END DO
         END DO
         DO jj = 2, jpjm1
            DO ji = fs_2, fs_jpim1   ! vector opt.
               ...
               !zck = ( gdepw_n(ji,jj,jk) - gdepw_n_tmp(ji,jj)) / MAX( hmlp(ji,jj) - gdepw_n_tmp(ji,jj), 10._wp )
...
# endif
# if defined slave
      call athread_spawn(slave_slp5, v5)
      call athread_join()
# else
      DO jk = 2, jpkm1
         DO jj = 2, jpjm1!, MAX(1, jpj-3)                        ! rows jj=2 and =jpjm1 only
            DO ji = 2, jpim1
               ...
               wslpi_tmp = (         zwz(ji-1,jj-1,jk) + zwz(ji+1,jj-1,jk)     &
                    &         +      zwz(ji-1,jj+1,jk) + zwz(ji+1,jj+1,jk)     &
                    &         + 2.*( zwz(ji  ,jj-1,jk) + zwz(ji-1,jj  ,jk)     &
                    &         +      zwz(ji+1,jj  ,jk) + zwz(ji  ,jj+1,jk) )   &
                    &         + 4.*  zwz(ji  ,jj  ,jk)                         ) * zcofw

               wslpj_tmp = (         zww(ji-1,jj-1,jk) + zww(ji+1,jj-1,jk)     &
                    &         +      zww(ji-1,jj+1,jk) + zww(ji+1,jj+1,jk)     &
                    &         + 2.*( zww(ji  ,jj-1,jk) + zww(ji-1,jj  ,jk)     &
                    &         +      zww(ji+1,jj  ,jk) + zww(ji  ,jj+1,jk) )   &
               ...
               wslpi(ji,jj,jk) = wslpi_tmp * zck
               wslpj(ji,jj,jk) = wslpj_tmp * zck
            END DO
         END DO
         !DO jj = 3, jpj-2                               ! other rows
         !   DO ji = fs_2, fs_jpim1   ! vector opt.
