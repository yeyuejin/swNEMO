...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: dynspg_ts.F90 12206 2019-12-12 11:14:55Z smasson $
...
CONTAINS
   ...
   SUBROUTINE dyn_spg_ts( kt )
      ...
      !!----------------------------------------------------------------------
      real(wp), dimension(jpi,jpj,jpk) :: check_tmp3d1,check_tmp3d2,check_tmp3d3, &
                  check_tmp3d4
      integer :: tag                 
#if defined slave 
      integer*8, save,volatile :: spgts4_sig=0, spgts5_sig=0
      external :: cyc_time, slave_spgts1, slave_spgts2, slave_spgts3, &
                  slave_spgts4, slave_spgts5
      integer*8 :: ct1,ct2,ct3,ct4
      type var_dynspg_ts
        integer :: jpi, jpj, jpk, fstwo, fsjpim1, jpjm1, jpkm1, jn,kt, narea
        integer*8 :: loc_spgts4_sig, loc_spgts5_sig
        real(wp) :: grav, r1_2dt_b, za1, za2, za3
        integer*8 :: loc_zu_frc, loc_zv_frc, loc_e3u_n, loc_e3v_n, &
                     loc_ua, loc_va, loc_umask, loc_vmask, &
                     loc_r1_hu_n, loc_r1_hv_n, loc_zhu, loc_zhv, loc_un_b, &
                     loc_vn_b, loc_hu_n, loc_hv_n, loc_e2u, loc_e1v,       &
                     loc_zu_trd, loc_ssumask, loc_zv_trd, loc_ssvmask, &
                     loc_ua_b, loc_ub_b, loc_va_b, loc_vb_b,  &
                     loc_un_adv, loc_vn_adv, loc_un, loc_vn, loc_ubb_e, &
                     loc_ub_e, loc_un_e, loc_ua_e, loc_vbb_e, loc_vb_e, &
                     loc_vn_e, loc_va_e, loc_sshbb_e, loc_sshb_e, loc_sshn_e, &
                     loc_ssha_e, loc_ssha, loc_zhup2_e, loc_zhvp2_e

      endtype
      type(var_dynspg_ts) :: v
      ...
      v%narea = narea
      v%grav = grav
      v%loc_zu_frc = loc(zu_frc)
      v%loc_zv_frc = loc(zv_frc)
      v%loc_e3u_n = loc(e3u_n)
      v%loc_e3v_n = loc(e3v_n)
      v%loc_ua = loc(ua)
      v%loc_va = loc(va)
      v%loc_umask = loc(umask)
      v%loc_vmask = loc(vmask)
      v%loc_r1_hu_n = loc(r1_hu_n)
      v%loc_r1_hv_n = loc(r1_hv_n)
      v%loc_zhu = loc(zhu)
      v%loc_zhv = loc(zhv)
      v%loc_un_b = loc(un_b)
      v%loc_vn_b = loc(vn_b)
      ...
      v%loc_e2u = loc(e2u)
      v%loc_e1v = loc(e1v)
      v%loc_zu_trd = loc(zu_trd)
      v%loc_ssumask = loc(ssumask)
      v%loc_zv_trd = loc(zv_trd)
      v%loc_ssvmask = loc(ssvmask)
      v%loc_ua_b = loc(ua_b)
      v%loc_ub_b = loc(ub_b)
      v%loc_va_b = loc(va_b)
      v%loc_vb_b = loc(vb_b)
      v%loc_un_adv = loc(un_adv)
      v%loc_vn_adv = loc(vn_adv)
      v%loc_un = loc(un)
      v%loc_vn = loc(vn)
      v%loc_un_adv = loc(un_adv)
      v%loc_r1_hu_n = loc(r1_hu_n)
      ...
      v%loc_r1_hv_n = loc(r1_hv_n)
      v%loc_vn_b = loc(vn_b)
      v%loc_ubb_e = loc(ubb_e)
      v%loc_ub_e = loc(ub_e)
      v%loc_un_e = loc(un_e)
      v%loc_ua_e = loc(ua_e)
      v%loc_vbb_e = loc(vbb_e)
      v%loc_vb_e = loc(vb_e)
      v%loc_vn_e = loc(vn_e)
      v%loc_va_e = loc(va_e)
      v%loc_sshbb_e = loc(sshbb_e)
      v%loc_sshb_e = loc(sshb_e)
      v%loc_sshn_e = loc(sshn_e)
      v%loc_ssha_e =loc(ssha_e) 
      v%loc_ssha = loc(ssha)
      v%loc_zhup2_e = loc(zhup2_e)
      v%loc_zhvp2_e = loc(zhvp2_e)
      v%loc_spgts4_sig = loc(spgts4_sig)
      v%loc_spgts5_sig = loc(spgts5_sig)
#endif      
      !
...
!     zwdramp = 1._wp / (rn_wdmin2 - rn_wdmin1) ! more general ramp
      ...
      !                                   !  ---------------------------  !
#if defined slave
    call athread_spawn(slave_spgts1, v)
    call athread_join()
#else      
...
!!gm  Is it correct to do so ?   I think so...
      ...
      !                                         !* 2D Coriolis trends
#if defined slave
      call athread_spawn(slave_spgts2, v) 
      call athread_join()
#else      
      ...
      DO jn = 1, icycle                             !  sub-time-step loop  !
         ...
         !
#if defined slave
         call athread_spawn(slave_spgts5, v)
#endif
         ...
         !-------------------------------------------------------------------------!
#if defined slave
         spgts5_sig = spgts5_sig + 1 + (v%za1 - v%za1) + (v%za2 - v%za2) + &
                      (v%za3 - v%za3)
...
!jth do implicitly instead
         ...
         za1 = wgtbtp1(jn)                                    
         v%za1 = wgtbtp1(jn)
         call athread_spawn(slave_spgts4, v);
#endif
!jth implicit bottom friction:
         ...
         !                                             !  ----
#if defined slave
         spgts4_sig = spgts4_sig + 1 + ( v%za1 - v%za1 )
         call athread_join()
#else         
         ubb_e  (:,:) = ub_e  (:,:)
         ub_e   (:,:) = un_e  (:,:)
...
#if defined slave
     v%r1_2dt_b = r1_2dt_b
     call athread_spawn(slave_spgts3, v)
     call athread_join()
#else      
      !
