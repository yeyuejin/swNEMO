...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: sshwzv.F90 11414 2019-08-07 08:58:09Z jchanut $
...
CONTAINS
   ...
   SUBROUTINE wzv( kt )
      ...
      !!----------------------------------------------------------------------
# if defined slave
      integer, external :: slave_wzv
      type :: wzv_var
         integer   :: jpi, jpj, jpk, jpkm1, narea
         real(wp)  :: z1_2dt
         integer*8 :: loc_wn, loc_e3t_n, loc_hdivn, loc_e3t_a, loc_e3t_b, loc_tmask
      end type wzv_var
      type(wzv_var) :: w_v
      w_v%jpi = jpi
      w_v%jpj = jpj
      ...
      w_v%loc_wn = loc(wn)
      w_v%loc_e3t_n = loc(e3t_n)
      w_v%loc_e3t_a = loc(e3t_a)
      w_v%loc_e3t_b = loc(e3t_b)
      w_v%loc_tmask = loc(tmask)
      w_v%loc_hdivn = loc(hdivn)
# endif
      !
...
# if defined slave
         w_v%z1_2dt = z1_2dt
         call athread_spawn(slave_wzv, w_v)
         call athread_join()
# else
         DO jk = jpkm1, 1, -1                       ! integrate from the bottom the hor. divergence
            ! computation of w
