...
MODULE dynnxt
   ...
   !! $Id: dynnxt.F90 12366 2020-02-11 11:17:24Z jchanut $ 
...
CONTAINS
   ...
   SUBROUTINE dyn_nxt ( kt )
      ...
      !!----------------------------------------------------------------------
# if defined slave
      integer, external :: slave_dynnxt, slave_dynnxt2, slave_dynnxt3
      type :: dynnxt_var
         integer   :: jpi, jpj, jpk, jpjm1, jpkm1, narea
         integer   :: flag
         real(wp)  :: atfp
         integer*8 :: loc_e3u_a, loc_e3v_a, loc_ua, loc_va, loc_umask, loc_vmask, &
                      loc_r1_hu_a, loc_r1_hv_a, loc_ua_b, loc_va_b, loc_e3u_b, &
                      loc_e3v_b, loc_un, loc_vn, loc_ub, loc_vb, loc_un_b, &
                      loc_vn_b, loc_ub_b, loc_vb_b, loc_r1_hu_b, loc_r1_hv_b
      end type dynnxt_var
      ...
      d_v%loc_r1_hu_a = loc(r1_hu_a)
      d_v%loc_r1_hv_a = loc(r1_hv_a)
...
# if defined slave
         d_v%flag = 1;
         call athread_spawn(slave_dynnxt, d_v)
         call athread_join()
# else
...
# if defined slave
            d_v%atfp = atfp
            call athread_spawn(slave_dynnxt3, d_v)
            call athread_join()
# else
            DO jk = 1, jpkm1                              
               DO jj = 1, jpj
...
# if defined slave
            d_v%loc_e3u_a = loc(e3u_b)
            d_v%loc_e3v_a = loc(e3v_b)
            d_v%loc_ua = loc(ub)
            d_v%loc_va = loc(vb)
            d_v%loc_umask = loc(umask)
            d_v%loc_vmask = loc(vmask)
            d_v%loc_r1_hu_a = loc(r1_hu_n)
            d_v%loc_r1_hv_a = loc(r1_hv_n)
            d_v%loc_ua_b = loc(un_b)
            d_v%loc_va_b = loc(vn_b)
            d_v%flag = 2;
            call athread_spawn(slave_dynnxt, d_v)
            call athread_join()
# else
...
# if defined slave
      d_v%loc_e3u_a = loc(e3u_a)
      d_v%loc_e3v_a = loc(e3v_a)
      d_v%loc_e3u_b = loc(e3u_b)
      d_v%loc_e3v_b = loc(e3v_b)
      d_v%loc_un = loc(un)
      d_v%loc_vn = loc(vn)
      d_v%loc_ub = loc(ub)
      d_v%loc_vb = loc(vb)
      d_v%loc_umask = loc(umask)
      d_v%loc_vmask = loc(vmask)
      d_v%loc_un_b = loc(un_b)
      d_v%loc_ub_b = loc(ub_b)
      d_v%loc_vn_b = loc(vn_b)
      d_v%loc_vb_b = loc(vb_b)
      d_v%loc_r1_hu_a = loc(r1_hu_a)
      d_v%loc_r1_hv_a = loc(r1_hv_a)
      d_v%loc_r1_hu_b = loc(r1_hu_b)
      d_v%loc_r1_hv_b = loc(r1_hv_b)
      call athread_spawn(slave_dynnxt2, d_v)
      call athread_join()
# else
