...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: dynvor.F90 11536 2019-09-11 13:54:18Z smasson $
...
CONTAINS
   ...
   SUBROUTINE vor_ene( kt, kvor, pun, pvn, pua, pva )
      ...
      !!----------------------------------------------------------------------
# if defined slave
      integer, external :: slave_vor
      type :: vor_var
         integer   :: jpi, jpj, jpk, fs_two, fs_jpim1, jpjm1, jpkm1, narea
         real(wp)  :: r1_4
         integer*8 :: loc_ff_f, loc_e2v, loc_e1u, loc_r1_e1e2f, loc_e2u, loc_e1v, loc_r1_e1u, loc_r1_e2v, &
                    & loc_pvn, loc_pun, loc_pua, loc_pva
      end type vor_var
      type(vor_var) :: v_v
      v_v%jpi = jpi
      v_v%jpj = jpj
      v_v%jpk = jpk
      v_v%fs_two = fs_2
      v_v%fs_jpim1 = fs_jpim1
      v_v%jpjm1 = jpjm1
      v_v%jpkm1 = jpkm1
      v_v%narea = narea
      v_v%loc_ff_f = loc(ff_f)
      v_v%loc_e2v = loc(e2v)
      v_v%loc_e1u = loc(e1u)
      v_v%loc_r1_e1e2f = loc(r1_e1e2f)
      v_v%loc_e2u = loc(e2u)
      v_v%loc_e1v = loc(e1v)
      v_v%loc_r1_e1u = loc(r1_e1u)
      v_v%loc_r1_e2v = loc(r1_e2v)
      v_v%loc_pvn = loc(pvn)
      v_v%loc_pun = loc(pun)
      v_v%loc_pua = loc(pua)
      v_v%loc_pva = loc(pva)
...
# endif
      ...
      !
# if defined slave
      call athread_spawn(slave_vor, v_v)
      call athread_join()
# else
!      DO jk = 1, jpkm1                                 ! Horizontal slab
!         DO jj = 1, jpjm1
