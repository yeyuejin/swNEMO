...
MODULE iom_nf90
   ...
   !! $Id: iom_nf90.F90 11536 2019-09-11 13:54:18Z smasson $
      ...
      ELSE                                       !== the file doesn't exist ==!   (or we overwrite it)
         ...
         IF( ldwrt ) THEN              !* the file should be open in write mode so we create it...
            ...
            IF( snc4set%luse ) THEN
               ...
               ENDIF
# if defined zsqhdfs
               CALL iom_nf90_check(nf90_create( TRIM(cdname), IOR(imode, NF90_HADAFS), if90id ), clinfo)
# else
               CALL iom_nf90_check(NF90_CREATE( TRIM(cdname), imode, if90id ), clinfo)
# endif
            ELSE
# if defined zsqhdfs
               CALL iom_nf90_check(nf90_create( TRIM(cdname), IOR(imode, NF90_HADAFS), if90id, chunksize = ichunk ), clinfo)
# else
