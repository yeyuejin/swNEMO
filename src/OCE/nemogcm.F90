...
MODULE nemogcm
   ...
   !! $Id: nemogcm.F90 12214 2019-12-12 15:46:06Z smasson $
...
CONTAINS
   ...
   SUBROUTINE nemo_gcm
      ...
      REAL(dp)::   zstptiming   ! elapsed time for 1 time step
      real(8) ::   stp_st, step_time, sum_time ! for core time
      external :: crts_init
      integer :: print_flag
...
# if defined slave
      !call athread_init()
      call crts_init()
      if(lwp) print *, 'slave mode'
# else
      !if(lwp) print *, 'master mode'
      if(lwp) print *, 'slave mode'
# endif
# if defined time
      if(lwp) sum_time = 0
...
# endif
            CALL stp        ( istp ) 
# if defined time
            step_time = MPI_Wtime() - stp_st
            sum_time = sum_time + step_time
            if(lwp) print *, istp, step_time, sum_time
# endif
            istp = istp + 1
...
# endif
   ...
   SUBROUTINE nemo_init
      ...
      !
#if defined zsqhdfs
      ! CALL  Hadafs I/O Interface
        ret = 0
        CALL hadafs_mount("group1", mpprank, ret, 6)
        IF ( ret .EQ. -1 ) THEN
          PRINT *, "VMP mounting failed!"
          stop 2
        ENDIF
      !
#endif
