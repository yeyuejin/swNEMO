...
CONTAINS
   ...
   SUBROUTINE p2z_bio( kt )
      ...
      !
! yyj --> 
      real(wp), dimension(jpi,jpj, jpk) :: check_tmp1, check_tmp2, check_tmp3, &
              check_tmp4, check_tmp5, check_tmp6
      real(wp), dimension(jpi,jpj) :: check_tmp2d              
      real(wp) :: sub_tmp,div_tmp
      integer :: tag        
      integer*8 :: ct1,ct2,ct3,ct4
      external :: cyc_time
#if defined slave
      external :: slave_bio1, slave_bio2
      type p2zbio_var
        integer :: jpi, jpj,jpk, jpkb, jpkbm1, jpkm1, jpjm1, fsjpim1,    &
                   fs2, jpdet, jpzoo, jpphy, jpno3, jpnh4, jpdom, narea
        real*8  :: aki, psinut, tmumax, rgamma, fphylab, rppz, taus, &
                   aks, rpnaz, rdnaz,tauzn , fzoolab, tmminp, tmminz,     &
                   fdbod, taudn, fdetlab, redf, reddom, taunn, akno3, &
                   aknh4, taudomn  
        integer*8 :: loc_trn, loc_etot, loc_tra, loc_xksi, loc_e3t_n
      endtype
      ...
      !call cyc_time(ct1)
      call athread_spawn(slave_bio1, v)
      call athread_join()
      !call cyc_time(ct2)
#else
      ...
      !
#if defined slave
      call athread_spawn(slave_bio2, v);
      call athread_join()
#else      
      ! -------------------------- !
