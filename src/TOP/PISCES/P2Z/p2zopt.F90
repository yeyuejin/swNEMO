...
# if defined p2z_time
      real(8) :: st, ed
      real(8), dimension(3) :: elapsed
      integer :: print_flag
      common /flag/ print_flag
# endif
# if defined slave
      integer, external :: slave_p2z1, slave_p2z2
      type :: p2d_var
         integer :: jpi, jpj, jpk, jpkm1, narea, kt, jpphy
         real(wp) :: zcoef, xkr0, xkrp, xlr, xkg0, xkgp, xlg, tiny0
         integer*8 :: loc_trn, loc_zparr, loc_e3t_n, loc_zparg, loc_etot
      end type p2d_var
      type(p2d_var) :: p_v
      p_v%jpi = jpi
      p_v%jpj = jpj
      ...
      p_v%xlg = xlg
      p_v%loc_trn = loc(trn)
      p_v%loc_zparr = loc(zparr)
      p_v%loc_e3t_n = loc(e3t_n)
      p_v%loc_zparg = loc(zparg)
      p_v%loc_etot = loc(etot)
      p_v%tiny0 = TINY(0.)
# endif
      !
...
# endif
# if defined slave
      call athread_spawn(slave_p2z1, p_v)
      call athread_join()
# else
...
# endif
# if defined slave
      call athread_spawn(slave_p2z2, p_v)
      call athread_join()
# else
