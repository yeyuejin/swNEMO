...
! yyj -->
      integer :: tag 
      real(wp) , dimension (jpi,jpj,jpk):: check_tmp
      real(wp) , dimension (jpi,jpj):: check_tmp2d, check_tmp2d1
      integer*8 :: ct1,ct2,ct3,ct4
      external :: cyc_time
      integer :: cnt, const_ikt
#if defined slave
      integer,volatile :: p2zexp1_sig
      external :: slave_p2zexp1, slave_p2zexp2, slave_p2zezp3
      type p2zexp_var
        integer :: jpi,jpj,jpk,fs2,fsjpim1,jpjm1,jpkm1,narea
        integer :: jpno3, jpdet, ikt
        real*8 :: sedlam, vsed, rdt, sedlostpoc, zgeolpoc, areacot
        integer*8 :: loc_e3t_n, loc_tra, loc_dmin3, loc_xksi, loc_mbkt, &
                     loc_sedpocn, loc_trn, loc_zsedpoca, loc_dminl,     &
                     loc_e1e2t, loc_cmask
        integer*8 :: loc_p2zexp1_sig
      endtype
      ...
      v%loc_tra = loc(tra)
      v%loc_dmin3 = loc(dmin3)
      v%loc_xksi = loc(xksi)
      v%loc_mbkt = loc(mbkt)
      v%loc_sedpocn = loc(sedpocn)
      v%loc_trn = loc(trn)
      v%loc_zsedpoca = loc(zsedpoca)
      v%loc_dminl = loc(dminl)
      v%loc_e1e2t = loc(e1e2t)
      v%loc_cmask = loc(cmask)
      v%loc_p2zexp1_sig = loc(p2zexp1_sig)

      zgeolpoc = 0.e0         !     Initialization
      v%sedlam = sedlam
      v%vsed = vsed
      v%rdt = rdt
      v%sedlostpoc = sedlostpoc
      v%zgeolpoc = zgeolpoc
      v%areacot = areacot
      
      p2zexp1_sig = 0
      call athread_spawn(slave_p2zexp1, v)
#endif      
      !!---------------------------------------------------------------------
      zsedpoca(:,:) = 0.
      !
...
!      call cyc_time(ct1)
      call athread_join()
!      call cyc_time(ct2)
!      if(lwp) print*, "DMA: ", (4*jpkm1+64)*jpi*jpj*8*2.21/(ct2-ct1), ct2-ct1
#else      
      ...
      !         check_tmp(ji,jj,jk) = check_tmp(ji,jj,jk) + ze3t * dmin3(ji,jj,jk) * xksi(ji,jj)
            END DO
         END DO
      END DO
      ...
      !do jj=1, jpjm1
      !do ji=fs_2, fs_jpim1
      !  if(check_tmp(ji,jj,jk) .ne. tra(ji,jj,jk,jpno3)) then
      !    if(tag == 1) then
      !      if(lwp) print*,"CAL ERR", ji,jj,jk,check_tmp(ji,jj,jk), &
      !        tra(ji,jj,jk,jpno3)
      !      tag =1
      !    endif
      !  endif
...
#if defined slave
! ------ ikt /= 1
!      check_tmp(:,:, mbkt(fs_2,2)) = tra(:,:, mbkt(fs_2,2), jpno3)
!      check_tmp(:,:, 1) = tra(:,:, 1, jpno3)
      v%ikt = mbkt(fs_2,2)
      call athread_spawn(slave_p2zexp2, v)
      call athread_join()
